# -*- coding: utf-8 -*-
"""
Created on Wed Jun 21 11:47:40 2023
...
def acelera(t,x,v):
    F_x = - k * x * ( 1 + 2 * beta * x**2)
    F_res = -b * v
    if(t >= 400): 
        F_ext = F0 * np.cos(1.37 * t)
    else:
...
for i in range(Nt-1):
    ...
    Ep[i+1] = 1/2 * k * x[i+1]**2 * (1 + beta * x[i+1]**2)
...
k = 0
i = np.where(t == 800)[0][0]     # Depende de quando começa o regime estacionário
x_ext = []
t_ext = []
...
plt.savefig(dir_figs+'ex2-c).png') # Save the graph in png
